...
void Layouter::execute(){
   ...
   mov.resize(num);
   for (s=0;s<(int) program.size();s++){
      int cc=0;
      bool end=false;
      while (!end){
         for (p=0;p<(int) plugins.size();p++){
            pg_mov.assign(num,Point(0,0));
            if (program[s].actplugins[p]==0.0) continue;
            plugins.get(p).pfunc(*this,plugins.get(p),pg_mov,cc,temp);
            for (i=0;i<num;i++){
               mov[i]=mov[i]+pg_mov[i]*program[s].actplugins[p];
...
double calc_force_adj(){
   ...
   */
   double force=0.0;
   double d,i_d,alpha,i_alpha,beta;
...
//         if(_type==substrate)mov[n2].y+=(i_d/n); //substrates at top;
      ...
      if(_type==substrate){
         ...
         double i_alpha1=PI+pts_dir[n1], beta1=lim(i_alpha1-alpha);
   ...
   */
   ...
   for(i=0;i<m;i++){
      ...
      if(_type==substrate){
         ...
         double i_alpha1=PI+pts_dir[p1], beta1=lim(i_alpha-alpha);
         if(fabs(beta)>fabs(beta1)){
            beta=beta1;
            i_alpha=i_alpha1;
         }
...
double firm_distribution(){
   ...
   */
   ...
   double strength=0.2; //this should not be a major force for reactions , so we make it small.
   VI *neighbors;
   double average,beta,beta2,d,force=0.0;
   Point baseNode,vec;
   for(k=0;k<n;k++){
   ...
   */
   ...
   for(i=0;i<m;i++){
      ...
      if(_type==product){
         ...
         double width=_getwidths(this,nd); // sum of width of all products
         ...
         double width=_getwidths(this,nd); // sum of width of all substrates
         double left=width*(sz-1)/sz;
         double step=0;
         if (sz>1) step=2*left/(sz-1);
         mov[n1].y+=(pos[n2].y-dij1[i]);
